...
class ScalingSAC(nn.Module):
    ...
    def __init__(
        ...
        super(ScalingSAC, self).__init__()
        self.env = env
        ...
        if cfg.net == "ScalingTransformerConv":
            from src.nets.ScalingTransformerConv import GNNActor, GNNCritic
    ...
    def learn(self, cfg, Dataset=None):
        ...
        for i_episode in epochs:
            ...
            while not done:
                ...
                action_dict = {"scaling_action":action_rl}
            ...
            if self.wandb is not None:
                self.wandb.log({"Reward": episode_reward, "Num Tasks finished": episode_num_tasks_finished, "Task search duration": np.mean(task_search_durations), "Task distance": np.mean(task_distances), "Q1 Loss": np.mean(self.LogQ1Loss), "Policy Loss": np.mean(self.LogPolicyLoss), "Q1": np.mean(self.LogQ1)}, step=i_episode)
