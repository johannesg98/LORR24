...
# Update the import path to directly reference the helperfunctions folder
sys.path.append(os.path.join(script_dir, "../RL"))
...
for i in range(number_of_runs):
    ...
    while not done:
        ...
        # action_rl = np.array([action_rl]).squeeze()
        # action_rl = np.random.rand(env.nNodes)
        # action_rl = action_rl / action_rl.sum()
        # action_rl = action_rl.squeeze()
