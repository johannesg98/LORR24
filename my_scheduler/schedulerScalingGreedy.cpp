...
namespace schedulerScalingGreedy{
...
void schedule_plan(int time_limit, std::vector<int> & proposed_schedule,  SharedEnvironment* env, const std::unordered_map<std::string, pybind11::object>& action_dict)
{
    ...
    int min_task_i, min_task_makespan, dist, c_loc, count, t_loc, t_node, a_node;
    clock_t start = clock();


    bool scaling_active = false;
    std::vector<std::vector<double>> scaling_action;
    if (action_dict.find("scaling_action") != action_dict.end()) {
        scaling_action = action_dict.at("scaling_action").cast<std::vector<std::vector<double>>>();
        scaling_active = true;        
    }
