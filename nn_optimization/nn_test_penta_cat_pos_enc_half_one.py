...
from nets.penta_cat_pos_enc_half_one import GNNActor
        ...
        with torch.no_grad():  # No need to compute gradients during evaluation
            for batch_idx, (obs, actions) in enumerate(test_loader):
                ...
                for i in range(len(actions)):  # len(actions) should be equal to the batch size
                    ...
                    onePredSum = one_pred_action.cpu().numpy().sum()
                    if onePredSum > 0.99 and onePredSum < 1.01:
...
for i in range(n_experiments):
    ...
    name = "penta_cat_pos_enc_half_one"
