...
class GNNActor(nn.Module):
    def __init__(self, in_channels, hidden_size=32, act_dim=6, edge_limit=0.5, out_channel_fac=2):
        ...
        self.conv2 = TransformerConv(self.conv2_in_channels, self.conv2_out_channels, edge_dim=1)
        self.lin4 = nn.Linear(self.conv2_in_channels + self.conv2_out_channels, self.last_hidden)
        self.lin5 = nn.Linear(self.last_hidden, self.last_hidden)
        self.lin6 = nn.Linear(self.last_hidden, 1)
        ...
        self.lin4_norm = nn.LayerNorm(self.last_hidden)
        self.lin5_norm = nn.LayerNorm(self.last_hidden)
    ...
    def forward(self, state, edge_index, deterministic=False, return_dist=False, return_raw=False):
        ...
        for i in range(x.shape[0]):
            data_list.append(Data(x=x[i], edge_index=self.edge_index_distancebased, edge_attr=self.edge_weights))
        batch = Batch.from_data_list(data_list)

        out2 = F.relu(self.conv2_norm(self.conv2(batch.x, batch.edge_index, edge_attr=batch.edge_attr)))
        out2 = out2.reshape(-1, self.act_dim, self.conv2_out_channels)
        x = torch.cat((out2, x), dim=-1)

        x = F.leaky_relu(self.lin4_norm(self.lin4(x)))
        x = F.leaky_relu(self.lin5_norm(self.lin5(x)))

        x = F.softplus(self.lin6(x))
