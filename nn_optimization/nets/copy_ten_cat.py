...
class GNNActor(nn.Module):
    def __init__(self, in_channels, hidden_size=32, act_dim=6):
        ...
        self.conv6 = GCNConv(in_channels, in_channels)
        self.conv7 = GCNConv(in_channels, in_channels)
        self.conv8 = GCNConv(in_channels, in_channels)
        self.conv9 = GCNConv(in_channels, in_channels)
        self.conv10 = GCNConv(in_channels, in_channels)
        self.lin1 = nn.Linear(11*in_channels, hidden_size)
    ...
    def forward(self, state, edge_index, deterministic=False, return_dist=False, return_raw=False):
        ...
        out6 = F.relu(self.conv6(out5, edge_index))
        out7 = F.relu(self.conv7(out6, edge_index))
        out8 = F.relu(self.conv8(out7, edge_index))
        out9 = F.relu(self.conv9(out8, edge_index))
        out10 = F.relu(self.conv10(out9, edge_index))
        if torch.isnan(out7).any():
            print("NaN values detected in out!")
        x = torch.cat((out1, out2, out3, out4, out5, out6, out7, out8, out9, out10, state), dim=-1)
