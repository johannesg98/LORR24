...
from nets.TransformerConvDouble2 import GNNActor
...
def do_one_training(dataset, batch_size = 32, lr = 0.001, num_epochs = 200, loss_fn = nn.MSELoss(), perc_data_used = 1, wandb_dict = None, multiStepLr = None, hidden_size=256, edge_limit=0.5, out_channel_fac=2):
    # Unpack the dataset
    ...
    model = GNNActor(in_channels=in_channels, hidden_size=hidden_size, act_dim=nNodes, edge_limit=edge_limit, out_channel_fac=out_channel_fac)  # Action dim is 79
...
def do_one_configuration(edge_limit, out_channel_fac):
    n_experiments = 4
    for i in range(n_experiments):
        ...
        multiStepLr = None
        wandb_dict = {
            "project": "TransformerConvDouble2",
        }
...
def main():
    ...
    edge_limit_list = [0.5, 0.7, 1.0]
    out_channel_fac_list = [2]
    # edge_limit_list = [0.3]
    ...
    for edge_limit in edge_limit_list:
        for out_channel_fac in out_channel_fac_list:
            ...
            if run_training(edge_limit, out_channel_fac):
                return
...
def run_training(edge_limit, out_channel_fac):
    global iterator
    run_done = False
    if iterator == slurm_task_id:
        do_one_configuration(edge_limit, out_channel_fac)
        run_done = True
    else:
        iterator += 1
    return run_done
