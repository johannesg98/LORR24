...
build_path = os.path.join(script_dir, "League_of_Robot_Runners/envWrapper/build")
sys.path.append(build_path)

import envWrapper
from GRAND.RL.src.helperfunctions.LRRParser import LRRParser
...
def setup_model(cfg, env, parser, device):
    ...
    cfg = cfg.model
    if model_name == "sac":
        from GRAND.RL.src.algos.sac import SAC
...
@hydra.main(version_base=None, config_path=os.path.join(script_dir, "GRAND/RL/src/config/"), config_name="config")
def main(cfg: DictConfig):
    
    env = envWrapper.LRRenv(
        inputFile=os.path.join(script_dir, cfg.model.map_path),
        outputFile=os.path.join(script_dir, "GRAND/outputs/GRAND_train.json"),
    ...
    if cfg.model.tensorboard:
        ...
        model.tensorboard = SummaryWriter(os.path.join(script_dir, "GRAND/RL/logs/", cfg.model.checkpoint_path, datetime.now().strftime("%Y%m%d-%H%M%S")))
