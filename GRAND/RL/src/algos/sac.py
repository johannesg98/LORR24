...
from GRAND.Rebalancing.reb_flow_solver import solveRebFlow
import random
...
from GRAND.RL.src.helperfunctions.skip_actor import skip_actor
from GRAND.RL.src.helperfunctions.assign_discrete_actions import assign_discrete_actions
...
class SAC(nn.Module):
    ...
    def __init__(
        ...
        if cfg.net == "GCNConv":
            from GRAND.RL.src.nets.GCNConv import GNNActor, GNNCritic
        elif cfg.net == "GCNConvPenta":
            from GRAND.RL.src.nets.GCNConvPenta import GNNActor, GNNCritic
        elif cfg.net == "NNConv":
            from GRAND.RL.src.nets.NNConv import GNNActor, GNNCritic
        elif cfg.net == "GATConv":
            from GRAND.RL.src.nets.GATConv import GNNActor, GNNCritic
        elif cfg.net == "TransformerConv":
            from GRAND.RL.src.nets.TransformerConv import GNNActor, GNNCritic
        elif cfg.net == "TransformerConvDouble":
            from GRAND.RL.src.nets.TransformerConvDouble import GNNActor, GNNCritic
        elif cfg.net == "TransformerConvAction":
            from GRAND.RL.src.nets.TransformerConvAction import GNNActor, GNNCritic
    ...
    def learn(self, cfg, Dataset=None):
        if cfg.model.load_from_ckeckpoint:
            self.load_checkpoint(path=os.path.join(self.train_dir, f"GRAND/RL/ckpt/{cfg.model.checkpoint_path}.pth"))
        ...
        if cfg.model.visu_episode_list:
            curr_visu_idx = 0
            outputFile = os.path.join(self.train_dir, "GRAND/outputs/cont_outputs/", cfg.model.checkpoint_path, str(cfg.model.visu_episode_list[curr_visu_idx])+".json")
            os.makedirs(os.path.join(self.train_dir, "GRAND/outputs/cont_outputs/", cfg.model.checkpoint_path), exist_ok=True)
        ...
        for i_episode in epochs:
            ...
            while not done:
                ...
                if done and i_episode == train_episodes - 1:
                    np.save(os.path.join(self.train_dir, f"GRAND/outputs/pibt_wait_map_RL.npy"), np.array(info["pibt-wait-map"]))
                step+= 1
                myTimer.rest += myTimer.addTime()      
    ...
    def test_best_checkpoint(self, i_episode, cfg):
        ...
        if self.last_best_checkpoint is not None:
            self.load_checkpoint(path=os.path.join(self.train_dir, f"GRAND/RL/ckpt/{cfg.model.checkpoint_path}_best_{self.last_best_checkpoint}.pth"))
    ...
    def test_seperate(self, cfg):
        self.load_checkpoint(path=os.path.join(self.train_dir, f"GRAND/RL/ckpt/{cfg.model.load_test_checkpoint_path}.pth"))
    ...
    def load_checkpoint(self, path="ckpt.pth"):
        ...
        try:
            ...
            print("failed to load checkpoint EXCEPTION")
    ...
    def checkpoint_handler(self, i_episode, episode_num_tasks_finished, cfg):
        if i_episode == 0:
            ...
            self.last_best_checkpoint = None
        self.save_checkpoint(path=os.path.join(self.train_dir, f"GRAND/RL/ckpt/{cfg.model.checkpoint_path}.pth"))
        ...
        if i_episode > 20 and np.mean(self.last_20_episode_tasks_finished) > self.best_num_tasks_finished:
            ...
            self.save_checkpoint(
                path=os.path.join(self.train_dir, f"GRAND/RL/ckpt/{cfg.model.checkpoint_path}_best_{i_episode}.pth")
            )
            if self.last_best_checkpoint is not None:
                os.remove(os.path.join(self.train_dir, f"GRAND/RL/ckpt/{cfg.model.checkpoint_path}_best_{self.last_best_checkpoint}.pth"))
